...
static BN_ULONG *bn_expand_internal(const BIGNUM *b, int words)
{
    ...
    if (words > (INT_MAX / (4 * BN_BITS2))) {
        print("bn_expand_internal: words too big\r\n");
        return NULL;
    }
    if (BN_get_flags(b, BN_FLG_STATIC_DATA)) {
        print("bn_expand_internal: Invalid flag\r\n");
        return NULL;
    }
    a = malloc(words * sizeof(*a));
    if (a == NULL) {
        print("bn_expand_internal: Malloc failed\r\n");
        return NULL;
    }
...
BIGNUM *bn_expand2(BIGNUM *b, int words)
{
    if (words > b->dmax) {
        ...
        if (!a) {
            print("bn_expand_internal failure\r\n");
            return NULL;
        }
        if (b->d != NULL)
            bn_free_d(b, 1);
