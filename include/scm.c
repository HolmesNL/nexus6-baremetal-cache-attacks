...
void msleep(unsigned int t) {
  for (unsigned int i=0; i<t; i++) {
	for (unsigned int a=0;a<1000;a++) {
	  unsigned long tmp = 0xFFEEDDCC * a;
	}
  }
}
...
static u32 smc(u32 cmd_addr)
{
	...
	register u32 r1 asm("r1") = (_uintptr_t)&context_id;
...
{
	int ret;
	u32 cmd_addr = (u32)cmd; //virt_to_phys(cmd);
...
void scm_inv_range(unsigned long start, unsigned long end)
{
	...
	extern void v7_dma_inv_range(u32 start, u32 end);
	while (start < end) {
				...
				size_t scm_buf_length)
{
	...
	if (cmd_buf)
		_memcpy(scm_get_command_buffer(scm_buf), cmd_buf, cmd_len);
	...
	if (resp_buf)
		_memcpy(resp_buf, scm_get_response_buffer(rsp), resp_len);
		...
		void *resp_buf, size_t resp_len)
{
	...
	//pr_err("NEDFORINS: scm_call: svc_id: %08x, cmd_id: %08x, cmd_len: %d\n", svc_id, cmd_id, cmd_len);
